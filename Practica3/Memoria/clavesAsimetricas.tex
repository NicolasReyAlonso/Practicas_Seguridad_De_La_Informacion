...
RSA (\textit{Rivest-Shamir-Adleman}) es un algoritmo de criptografía asimétrica basado en la dificultad de factorizar el producto de 
dos números primos grandes. Permite tanto cifrado como firma digital.
...
La clave privada generada está cifrada con AES-256-CBC y protegida con la contraseña proporcionada. 
La cabecera del archivo PEM indica que está cifrada:
...
El formato DER es la representación binaria de la clave. Una característica importante es que \textbf{DER no soporta cifrado
 con contraseña}:
...
En lugar de \texttt{-----BEGIN ENCRYPTED PRIVATE KEY-----}, lo que demuestra que la conversión a formato DER \textbf{elimina la 
protección con contraseña} de la clave privada.
...
DSA (\textit{Digital Signature Algorithm}) es un algoritmo de firma digital estandarizado por el NIST. 
A diferencia de RSA, DSA \textbf{solo permite firmar}, no cifrar.
...
\textbf{Nota:} El tamaño de la firma DSA es variable (entre 46-48 bytes típicamente), ya que DSA codifica la firma como estructura 
ASN.1 con dos valores $(r, s)$.
...
El protocolo \textbf{Diffie-Hellman} permite a dos partes derivar un \textbf{secreto compartido} sin necesidad de transmitirlo. 
El principio se basa en la propiedad:
...
X25519 es una implementación del protocolo Diffie-Hellman usando la \textbf{curva elíptica Curve25519}, diseñada por Daniel J. Bernstein. 
Es el algoritmo utilizado por aplicaciones como \textbf{WhatsApp} y \textbf{Signal}.
...
\textbf{Conclusión:} X25519 genera un secreto de \textbf{32 bytes} (256 bits) frente a los \textbf{256 bytes} (2048 bits) de 
DH estándar, logrando seguridad equivalente con un secreto \textbf{8 veces menor}. 
Esto se debe a que la seguridad en curvas elípticas crece mucho más rápido con el tamaño de la clave que en DH clásico.
