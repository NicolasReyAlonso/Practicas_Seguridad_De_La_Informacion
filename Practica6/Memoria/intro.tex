...
Tal como exige el enunciado de la práctica, en todas las capturas de pantalla de la consola que aparecen en esta memoria se ha personalizado la variable de entorno \texttt{PS1} con el nombre y apellidos del estudiante. Para ello se ha utilizado la siguiente definición en el fichero de configuración del \emph{shell} (la máquina virtual Kali utiliza \texttt{zsh} por defecto):
\begin{lstlisting}[language=bash]
export PS1='%F{blue}Nicolas Rey Alonso-[%F{white}%~%F{blue}] $ %f'
\end{lstlisting}
De este modo, en todas las capturas de pantalla del sistema operativo que aparecen en esta memoria, el \emph{prompt} muestra de forma inequívoca el nombre del estudiante que ha realizado la práctica.
