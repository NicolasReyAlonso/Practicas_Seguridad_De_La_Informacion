...
\textbf{5.2. Decodificación manual del mensaje cifrado y firmado con OpenPGP utilizando la utilidad Gnupg}

\textbf{5.2.1. Prerrequisitos}
Para realizar este apartado es necesario tener instalada la utilidad ``gnupg'', es un paquete muy común
en distribuciones de Linux y en muchas está instalado por defecto.
También puede ser necesaria la utilidad ``ripmime'' de utilidades MIME, ya citada en la práctica 1.

\textbf{5.2.2 Secuencia de operaciones}
...
Descargar del OpenPGP Key Manager de Thunderbird una copia de nuestra clave privada, protegida
por contraseña (a un fichero \texttt{claveprivada.asc}).
Descargar también la clave pública del compañero que envió el mensaje (a un fichero \texttt{clave.asc}).
Exportar (guardar cómo\dots) el mensaje cifrado y firmado como ``\texttt{mensaje.eml}''.
...
caso lo ideal es utilizar la utilidad ``ripmime'' para extraer los adjuntos MIME del mensaje descifrado:
\begin{lstlisting}
# gpg --decrypt mensaje.eml > mensaje.mime
# mkdir dir_final
# ripmime mensaje.mime dir_final
\end{lstlisting}
Y esto nos crearía tantos ficheros como adjuntos tenga el mensaje en el ``directorio''.

\textbf{Nota importante:} la verificación de la firma del mensaje indicará que la clave del remitente no es
de confianza\dots dejamos como trabajo del estudiante aprender a gestionar la confianza de las
claves con Gnupg (list\_keys, edit\_key, trust $\rightarrow$ 5, etc), cosa que se hizo anteriormente en
Thunderbird, y demostrar que la firma se ha hecho con una clave a la que hemos conferido la
máxima confianza.
